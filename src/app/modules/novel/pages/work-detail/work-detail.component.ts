...
export class WorkDetailComponent implements OnInit, OnDestroy {
    ...
    constructor(public dialog: MatDialog,
        ...
        this.route.paramMap.subscribe( (paramMaps: any) => {
            ...
            this.episodeStore$.dispatch(EpisodeActions.FetchTitleAndNEpisodes({ params }));
        });
    }
    ...
    ngOnDestroy() {
        this.episodeStore$.dispatch(EpisodeActions.ResetNTitle());
        this.episodeStore$.dispatch(EpisodeActions.ResetNEpisodes());
    ...
    private setupReducerListener() {
        ...
        this.title$ = this.episodeStore$.pipe(select(fromEpisode.getNTitle), filter(title => !!title), takeUntil(this.destroyed$));
        this.episodes$ = this.episodeStore$.pipe(select(fromEpisode.getNEpisodes), takeUntil(this.destroyed$));
        this.firstEpisode$ = this.episodeStore$.pipe(select(fromEpisode.getFirstNEpisode), takeUntil(this.destroyed$));
        this.isFetching$ = this.episodeStore$.pipe(select(fromEpisode.getIsNFetching), takeUntil(this.destroyed$));
        this.episodePage$ = this.episodeStore$.pipe(
            select(fromEpisode.getNEpisodes),
        ...
        this.episodes$.subscribe(episode => {
            ...
            if (total && total > 0 && !this.firstEpisode) {
                this.episodeStore$.dispatch(EpisodeActions.FetchFirstNEpisode({ titleId: this.titleId }));
            }
        });
        this.firstEpisode$.subscribe(firstEpisode => {
            return this.firstEpisode = firstEpisode;
        });
    }
    ...
    private setupScrolledDownEventListener() {
        ...
        shouldFetch$.subscribe(page => {
            this.getEpisodes({titleId: this.titleId, page: page + 1, limit: 10, sort: this.sortType});
        });
    }

    private getEpisodes(params: any = {}) {
        this.episodeStore$.dispatch(EpisodeActions.FetchNEpisodes({ params }));
    }
        ...
        this.comicService.isAllcomicsAuth().toPromise().then(isAuth => {
            if (isAuth['body'].data.data.isValid) {
                ...
                this.comicService.getAllcomicsProfile().subscribe(res => {
                    ...
                    const payData = {
                        UserID: user_id,
                        ID: parseInt(episodeId.split('_')[0], 10),
                        EpisodeID: episodeId,
                        ContentType: 1,
    ...
    checkFirstRoute() {
        console.log('KSL', 'first', this.firstEpisode);
    ...
    onShowDetail(title: string, id: string) {
        this.router.navigateByUrl('/novel/view/' + title + '/' + id);
    }
