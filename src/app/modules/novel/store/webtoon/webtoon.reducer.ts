...
export const _webtoonReducer = createReducer(
    initialState,
    on(WebtoonActions.RequestNSuccess, (state) => ({
        ...state,
    })),
    on(WebtoonActions.SetNActiveGenre, (state, { genre }) => ({
        ...state,
        activeGenre: genre
    })),
    on(WebtoonActions.SetNovelList, (state, { genre, res }) => {
    ...
    on(WebtoonActions.SetNActiveGenreBanner, (state, { genre, banner }) => {
        state[genre] = { ...state[genre], banner };
        return { ...state };
    }),
    on(WebtoonActions.SetNGenresBannerList, (state, { total, list }) => ({
        ...state,
        bannerList: list
    })),
    on(WebtoonActions.SetIsNFetching, (state, { isFetching }) => ({
        ...state,
        isFetching
    })),
);
...
const selectWebtoon = createFeatureSelector<State>('novel');
